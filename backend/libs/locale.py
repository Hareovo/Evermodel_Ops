...
_PATTERNS = [
    ...
    (re.compile(r'^通过EVERMODEL_SET动态更新配置出错，未找到环境标识符(.*)$', re.S),
     'Failed to update configuration via EVERMODEL_SET: environment identifier {0} not found'),
    (re.compile(r'^通过EVERMODEL_SET动态更新配置出错，未找到应用或服务标识符(.*)$', re.S),
     'Failed to update configuration via EVERMODEL_SET: app or service identifier {0} not found'),
...
_CONSOLE_PATTERNS = [
    ...
    (re.compile(r'^检测到该主机的发布目录 (.*) 已存在，为了数据安全请自行备份后删除该目录，Evermodel Ops 将会创建并接管该目录。$', re.S),
     'The deploy directory {0} already exists on this host, please back it up and remove it yourself for data safety, Evermodel Ops will create and take over this directory.'),
...
def translate_console(text, language='zh'):
    """翻译发布/构建/流水线等实时控制台里由 Evermodel Ops 自身产生的文案。
