...
def wrap_python_command(command):
    ...
    """
    return (
        'EVERMODEL_PY=$(command -v python3 || command -v python)\n'
        'if [ -z "$EVERMODEL_PY" ]; then\n'
        '  echo "[Evermodel Ops] python interpreter not found on this host" >&2\n'
        '  false\n'
        'else\n'
        f"$EVERMODEL_PY << 'EVERMODEL_PY_EOF'\n# -*- coding: UTF-8 -*-\n{command}\nEVERMODEL_PY_EOF\n"
        'fi'
    )
...
def json_response(data='', error=''):
    ...
    return HttpResponse(json.dumps(content, cls=EvermodelEncoder, ensure_ascii=False), content_type='application/json')
...
class EvermodelEncoder(json.JSONEncoder):
    def default(self, o):
