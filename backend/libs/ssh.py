...
class SSH:
    def __init__(self, hostname, port=22, username='root', pkey=None, password=None, default_env=None,
        ...
        self.channel = None
        self.sftp = None
        self.exec_file = f'/tmp/evermodel.{uuid4().hex}'
        self.term = term or {}
        self.pid = None
        self.eof = 'Evermodel EOF 2108111926'
        self.default_env = default_env
        self.regex = re.compile(r'(?<!echo )Evermodel EOF 2108111926 (-?\d+)[\r\n]?')
        self.arguments = {
            'hostname': hostname,
            'port': port,
            'username': username,
    ...
    def _get_channel(self):
        ...
        while True:
            if self.channel.recv_ready():
                ...
                self.client.close()
                raise Exception('Wait evermodel_ops response timeout')
            else:
                counter += 1
                time.sleep(0.1)
        return self.channel
