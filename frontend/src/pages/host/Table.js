...
import { http, blobToExcel, humanDate, t } from 'libs';
    ...
    <TableCard
      ...
      actions={[
        ...
        <Button
          ...
          loading={loading}
          icon={<ExportOutlined/>}
          onClick={handleExport}>{t('导出')}</Button>,
        <Button
          type="primary"
          icon={<SyncOutlined/>}
          onClick={() => store.showSync()}>{t('验证')}</Button>,
      ...
      <Table.Column width={160} title={t('操作')} render={info => (
        <Action>
          ...
          <Action.Button danger onClick={() => handleDelete(info)}>{t('删除')}</Action.Button>
        </Action>
      )}/>
    </TableCard>
  )
