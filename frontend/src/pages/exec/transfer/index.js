...
import { Form, Button, Tooltip, Space, Card, Table, Input, Upload, message, Modal } from 'antd';
...
function TransferIndex() {
  ...
  function refreshHistories() {
    http.get('/api/exec/transfer/').then(res => setHistories(res))
  }
  ...
  function handleDeleteHistory(e, item) {
    ...
    Modal.confirm({
      ...
      onOk: () => http.delete('/api/exec/transfer/', {params: {id: item.id}}).then(refreshHistories)
    })
  }
  ...
  function handleClearHistory() {
    Modal.confirm({
      title: t('清空分发记录'),
      content: t('将删除当前账户的全部分发记录，删除后不可恢复。'),
      onOk: () => http.delete('/api/exec/transfer/', {params: {all: true}}).then(refreshHistories)
    })
  }
