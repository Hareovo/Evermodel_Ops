...
import { Modal, Tag, Dropdown, Radio, message, Button } from 'antd';
import { LinkButton, Action, TableCard } from 'components';
...
class ComTable extends React.Component {
  ...
  moreMenus = (info) => ({
    items: [
      {
        key: 'test',
        label: <LinkButton onClick={() => this.handleTest(info)}>{t('执行测试')}</LinkButton>
      },
      {
        key: 'active',
        label: (
      ...
      {
        key: 'delete',
        label: <LinkButton danger onClick={() => this.handleDelete(info)}>{t('删除')}</LinkButton>
      }
    ]
  });
      ...
      <TableCard
        ...
        actions={[
          <Button
            type="primary"
            icon={<PlusOutlined/>}
            onClick={() => store.showForm()}>{t('新建')}</Button>,
