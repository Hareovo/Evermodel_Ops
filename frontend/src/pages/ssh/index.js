...
import LogoEvermodelText from 'layout/logo-evermodel-white.png';
...
function WebSSH(props) {
  ...
  useEffect(() => {
    window.document.title = 'Evermodel Ops web terminal'
  ...
  const web_terminal_banner =
    '                  __       __                          _                __\n' +
    ' _      __ ___   / /_     / /_ ___   _____ ____ ___   (_)____   ____ _ / /\n' +
  ...
  return (
    <div className={styles.container} onMouseUp={handleMouseUp} onMouseMove={handleMouseMove}>
      ...
      <div className={styles.content}>
          ...
          tabBarExtraContent={hosts.length === 0 ? (
            ...
            <React.Fragment>
              <Button
                ...
                icon={<LeftOutlined/>}>{t('文件管理器')}</Button>
        ...
        {hosts.length === 0 && (
          <pre className={sshMode ? styles.fig : styles.fig2}>{web_terminal_banner}</pre>
        )}
      </div>
      <Drawer
        title={t('文件管理器')}
